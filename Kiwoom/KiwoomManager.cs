                ...
                if (e.sRQName.Length > 8)
                {
                    ...
                    if (e.sRQName.Substring(0, 8).Equals("주식기본정보요청"))
                    {
                        ...
                        if (str.Length == 2)
                        {
                            ...
                            편입알림메세지 += String.Format("https://t1.daumcdn.net/finance/chart/kr/daumstock/d/A{0}.png?t={1}", 종목코드, DateTime.Now.ToString("yyyyMMddhhmmss"));
                    ...
                    if (e.sRQName.Substring(0, 10).Equals("주식기본정보호가요청"))
                    {
                        ...
                        if (str.Length == 2)
                        {
                            ...
                            if (!조건식포착종목.ContainsKey(조건명 + 종목코드))
                            {
                                Console.WriteLine("편입종목호가 알림 메세지 이게 안뜬다고?");
        ...
        private void onReceiveRealCondition(object sender, _DKHOpenAPIEvents_OnReceiveRealConditionEvent e)
        {
            try {
                ...
                if (e.strType == "I")
                {
                    ...
                    {
                      Console.WriteLine(String.Format("{0} 편입함, 근데 이미 종목 포함되어 있음", data.종목명));
                      return;
                    }
                    조건식포착종목[e.strConditionName + data.종목코드] = "";

                    Console.WriteLine(String.Format("{0} 편입함", data.종목명));
                    api.SetInputValue("종목코드", data.종목코드);
                    api.CommRqData("주식기본정보요청;" + e.strConditionName, "opt10001", 0, GetScrNum());
